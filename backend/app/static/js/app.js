    ...
    container.innerHTML = `
        ...
        <div class="bg-gray-900/60 border border-gray-700 rounded-lg p-3">
            ...
            <div id="planner-targets-content" class="section-content" style="display:none">
                <div class="mb-2 mt-2">
                    <label class="text-xs text-gray-500 block mb-1">Draft Strategy Preset</label>
                    <select id="strategy-preset-select"
                        onchange="applyDraftStrategy(this.value)"
                        class="w-full px-2 py-1 bg-gray-900 border border-gray-700 rounded text-xs text-gray-200 focus:border-indigo-500">
                        <option value="">— Pick a strategy —</option>
                        ${Object.entries(DRAFT_STRATEGIES).map(([key, s]) =>
                            `<option value="${key}" ${key === 'balanced' ? 'selected' : ''}>${s.emoji} ${s.label}</option>`
                        ).join('')}
                    </select>
                    <p id="strategy-description" class="text-xs text-gray-500 mt-1 italic"></p>
                </div>
